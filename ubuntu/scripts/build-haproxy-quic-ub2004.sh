...
wget -c -t 0 -T 9 "https://github.com/PCRE2Project/pcre2/releases/download/pcre2-${_pcre2_ver}/pcre2-${_pcre2_ver}.tar.bz2"
...
sleep 2
cd ..
rm -fr "pcre2-${_pcre2_ver}"

cd "lua-${_lua_ver}"
sed 's#INSTALL_TOP=.*#INSTALL_TOP= /tmp/lua#g' -i Makefile
make linux
rm -fr /tmp/lua
sleep 1
make install
[ -d /tmp/lua/man ] && mv -f /tmp/lua/man /tmp/lua/share/
rm -fr /tmp/lua/share/lua
sleep 2
cd ..
rm -fr "lua-${_lua_ver}"
...
USE_SYSTEMD=1 \
USE_GETADDRINFO=1 \
ZLIB_LIB=/tmp/zlib/lib \
ZLIB_INC=/tmp/zlib/include \
SSL_LIB=/tmp/openssl/usr/lib/x86_64-linux-gnu \
SSL_INC=/tmp/openssl/usr/include \
PCRE2_LIB=/tmp/pcre2/lib \
PCRE2_INC=/tmp/pcre2/include \
LUA_LIB=/tmp/lua/lib \
LUA_INC=/tmp/lua/include \
...
getent passwd haproxy >/dev/null || useradd -r -g haproxy -d /var/lib/haproxy -s /usr/sbin/nologin -c "HAProxy Load Balancer" haproxy
...
echo
sleep 2
tar -Jcvf /tmp/"haproxy_${_haproxy_ver}-1_amd64.tar.xz" *
echo
sleep 2
cd /tmp
sleep 2
...
echo ' build haproxy done' >> /tmp/.done.txt
echo
/sbin/ldconfig
exit
